...
Original file is located at
    https://colab.research.google.com/drive/19OxcTnzhf6RblLgQq0xP2e0oCCt7jjcs
...
# El dataset es un archivo gzip, primero hay que descomprimirlo y luego pedimos que sea leído como un archivo CSV indicando 
# que los datos están separados por tabulación. Usamos para esta acción un metodo de pandas a través de este código para obtener los datos. 
...
# Tenemos filas que corresponden a genes de pollo, las dos primeras columnas nos dan información de cada gen y el resto de las columnas 
# corresponden las réplicas biólogicas de cada población celular secuenciada por RNA-seq que contienen las counts registradas para cada gen.

"""# Procesado del dataset"""

# Para trabajar mejor y reducir errores, primero vamos a realizar una copia de la tabla y 
# a partir de la nueva tabla vamos a tratarla para que nos quede lista para el analisis de expresión de genes. 

data_copia = data.copy()

# Aclaración: Las modificaciones tienen que tener efecto en nuestro DataFrame "data_copia" 
# asi que vamos a incluir **inplace=True** en los codigos que sean necesario*.

# Eliminar las columnas que corresponden a non NC en un sólo paso porque no las vamos a utilizar:
...
# Usamos rename, para renombrar las columnas.
...
# Reordenamos las columnas para que las replicas de cada población queden juntas.
...
# Normalizamos loscounts utilizando el método CPM
nm = norm()
...
# Ejecutar el siguiente código para realizar el agrupamiento:
...
Nosotros queremos evaluar cómo la expresión de los genes en las células migratorias con respecto a las pre  migratorias. Entonces, nuestro "tratamiento" es población migratoria y nuestro "control" es población pre migratoria. 
...
#Eliminamos las filas que tienen counts = 0 en ambas poblaciones
cpm_df.drop(cpm_df[(cpm_df['PM_mean'] == 0) & (cpm_df['M_mean'] == 0)].index, inplace=True)

#La operación para el log2FC es la siguiente:
# log2FC = np.log2((cpm_df["M_mean"])/(cpm_df["PM_mean"]))

# Agregamos una columna a nuestra tabla que contenga el log2FC de cada gen.
...
# Crear dos grupos donde incluimos las replicas para de las poblaciones en cada uno. 
PM = [cpm_df['1_PM'], cpm_df['2_PM'], cpm_df['3_PM'], cpm_df['4_PM']]
M = [cpm_df['1_M'], cpm_df['2_M'], cpm_df['3_M'], cpm_df['4_M']]

# Calculamos la varianza de cada grupo para determinar si asumimos que nuestras dos poblaciones tienen varianzas iguales.  
print(np.var(PM), np.var(M))

#Podemos asumir que las poblaciones tienen varianzas iguales si: 
# varianza de la muestra más grande/la varianza de la muestra < 4. Si esto pasa realizamos realice una prueba t estándar independiente de 2 muestras que asume varianzas poblacionales iguales.**
print(np.var(M)/np.var(PM))

# Con este codigo vamos a generar un DataFrame que contenga el valor estadistico y el p-value. Utilizamos *equal_var = True* porque asumimos varianzas poblaciones iguales.
...
# Ordenamos los datos para que los genes con el p-value más bajo nos queden en las primeras filas. Y a partir de DataFrame vamos a recuperar el Geneid de los 10 primeros para una posterior búsqueda. 

viz_sort = viz_df.sort_values("p-val")
list(viz_sort["Geneid"].iloc[0:10])

# Volcano plot para ver la distribución de nuestros datos relacionando el log2FC con el p-value de los genes. 
